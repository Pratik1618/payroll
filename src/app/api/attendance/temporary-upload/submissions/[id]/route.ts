      ...
      (res) => {
        ...
        const finish = () => {
          ...
          try {
            ...
            console.error('[submission preview] Decompression failed, falling back to raw:', err?.message)
            body = buffer.toString('utf-8')
          }

          console.log('[submission preview] Finished. Transfer length:', buffer.length, 'Uncompressed length:', body.length, 'status:', statusCode)
...
function repairJson(raw: string): any {
  try {
    ...
    try {
      ...
      console.log('[submission preview] Repaired truncated JSON successfully')
      return result
    } catch (e2) {
...
export async function GET(
  ...
  try {
    ...
    console.log('[submission preview] Fetching:', backendUrl)
    ...
    try {
      ...
      console.error('[submission preview] JSON parse/repair failed. Last 100 chars:', body.substring(body.length - 100))
      return NextResponse.json({ message: 'Invalid response from backend' }, { status: 502 })
    }

    console.log('[submission preview] Success!')
    return NextResponse.json(data, { status })
  } catch (error: any) {
    console.error('[submission preview] Error:', error?.message || error)
