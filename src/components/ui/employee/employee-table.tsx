...
interface Employee {
  ...
  // Tax regime fields
  taxRegime?: "Old" | "New"
  deductionsAnnual?: number // annual deductions (for Old regime)
}
...
const mockEmployees: Employee[] = [
  {
    ...
    // tax fields
    taxRegime: "Old",
    deductionsAnnual: 100000, // example: investments + other deductions
  },
  {
    id: "2",
    name: "Priya Sharma",
...
// Tax slabs (FY 2025-26 simplified)
const taxSlabs = {
  Old: [
    { upto: 250000, rate: 0 },
    { upto: 500000, rate: 5 },
...
const calculateAnnualTax = (annualIncome: number, regime: "Old" | "New", deductions = 0) => {
...
export function EmployeeTable({ searchTerm, filterSite }: EmployeeTableProps) {
  ...
  const updateTaxRegime = (id: string, regime: "Old" | "New") => {
    setEmployees((prev) => prev.map(emp => emp.id === id ? { ...emp, taxRegime: regime } : emp))
  }
      ...
      <table className="w-full">
        <thead>
          <tr className="border-b border-border">
            ...
            <th className="text-left py-3 px-4 font-medium text-muted-foreground">Tax Regime</th>
            <th className="text-left py-3 px-4 font-medium text-muted-foreground">Monthly Tax</th>
        ...
        <tbody>
          {filteredEmployees.map((employee) => {
            const annualIncome = ((employee.basic || 0) + (employee.da || 0) + (employee.hra || 0)) * 12
            const deductionsAnnual =
              employee.deductionsAnnual ??
              ((employee.pfDeduction || 0) + (employee.esicDeduction || 0) + (employee.ptDeduction || 0)) * 12
            const taxOld = calculateAnnualTax(annualIncome, "Old", deductionsAnnual)
            const taxNew = calculateAnnualTax(annualIncome, "New", 0)
            const chosenAnnualTax = (employee.taxRegime === "Old" ? taxOld : taxNew) || 0
            const monthlyTDS = Math.round(chosenAnnualTax / 12)
            ...
            return (
              <tr key={employee.id} className="border-b border-border hover:bg-accent/50">
                ...
                {/* Tax regime selector (REPLACED: read-only display) */}
                <td className="py-3 px-4">
                  <Badge
                    className={
                ...
                <td className="py-3 px-4 text-foreground">₹{monthlyTDS.toLocaleString()}</td>
                      ...
                      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto rounded-xl shadow-lg border border-border bg-background">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm mt-2">
                          ...
                          <div className="rounded-lg bg-muted/50 p-4 border col-span-1 md:col-span-2">
                            <div className="flex items-center gap-2 mb-2 text-primary font-semibold">
                              <Wallet className="h-4 w-4" /> Salary & Tax Details
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              ...
                              <div>
                                ...
                                <div className="font-medium mt-2 mb-1">Tax</div>
                                <div>Tax Regime: {employee.taxRegime || "Old"}</div>
                                <div>Annual Tax (Old): ₹{taxOld.toLocaleString()}</div>
                                <div>Annual Tax (New): ₹{taxNew.toLocaleString()}</div>
                                <div className="font-medium mt-2">Estimated Monthly TDS: ₹{monthlyTDS.toLocaleString()}</div>
                              </div>
                            </div>
                          </div>
